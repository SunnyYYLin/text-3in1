...
class TranslationDataCollator(DataCollatorMixin):
    ...
    def __call__(self, features: list[dict[str, list[int]]]) -> dict[str, torch.Tensor]:
        src_ids = [[SRC_GO_ID]+f["src_ids"]+[SRC_EOS_ID] for f in features]
        tgt_ids = [[TGT_GO_ID]+f["tgt_ids"]+[TGT_EOS_ID] for f in features]
        ...
        padded_src_ids = torch.full((batch_size, src_max_len), SRC_PAD_ID, dtype=torch.long)
        padded_tgt_ids = torch.full((batch_size, tgt_max_len), TGT_PAD_ID, dtype=torch.long)
